"""experiments/EchoNet/02_echonet.py

Evaluates EchoPrime classification confidence degradation on EchoNet (A2C/A4C)
...
Output: assets/results/EchoNet/{model}/{tgt}.npz
...
# EchoPrime 11-class indices for the two EchoNet views
EchoNet_ECHOPRIME_LABEL = {
    'A2C': 0,  # A2C is class 0 in COARSE_VIEWS
    'A4C': 2,  # A4C is class 2 in COARSE_VIEWS
}

RAW_BASE = 'assets/data/EchoNet/raw'
AUG_BASE = 'assets/data/EchoNet/aug'
SAVE_BASE = 'assets/results/EchoNet'
...
class EchoNetDataset(Dataset):
    ...
    def __init__(self, tgt: str, aug_variant: str | None):
        self.label = EchoNet_ECHOPRIME_LABEL[tgt]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='EchoPrime confidence degradation on EchoNet under EchoGains augmentations.'
    )
    ...
    parser.add_argument('--tgt', type=str, default='A4C', choices=['A4C'])
    ...
    for tgt in tgts:
        ...
        label = EchoNet_ECHOPRIME_LABEL[tgt]

        print(f'\n=== {tgt} | discovered variants: {variants} ===')

        n_images = len(EchoNetDataset(tgt, None))
