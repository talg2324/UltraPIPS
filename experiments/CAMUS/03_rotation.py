...
src_dir = 'experiments/assets/data/CAMUS/raw'
dst_dir = 'experiments/assets/data/CAMUS/aug'

parser = ArgumentParser()
parser.add_argument("--aug-power", type=int, required=True)
parser.add_argument("--tgt", choices=['A4C', 'A2C'], required=True)
...
if __name__ == "__main__":
    ...
    power_suffix = f'rotation-{args.aug_power}'
    ...
    print(f"Preparing masks for {args.tgt} {batch_suffix} (power={args.aug_power})")
    for s in subsets:
        val = configs[s]['val']
        ...
        params = [{'type': 'rotation', 'prob': 1, 'rotation_range': [val, val + 1]}]
